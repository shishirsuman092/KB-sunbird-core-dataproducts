error id: file://<WORKSPACE>/batch-models/src/main/scala/org/ekstep/analytics/dashboard/report/user/UserReportModel.scala:`<error>`#`<error>`.
...
non-local guesses:
	 -org/apache/spark/sql/functions/userExtendedProfileDF2.
	 -org/ekstep/analytics/dashboard/DashboardUtil.userExtendedProfileDF2.
	 -org/ekstep/analytics/dashboard/DataUtil.userExtendedProfileDF2.
	 -userExtendedProfileDF2.
	 -scala/Predef.userExtendedProfileDF2.
offset: 10079
...
//    }
    ...
    userExtended@@ProfileDF2.show(false)

    // Step 1: Explode customFieldValues to get individual attribute-value pairs
    val explodedDF = userExtendedProfileDF2
      .withColumn("customField", explode(col("contextData.customFieldValues")))
      .select(
        col("userID"),
        col("organisationId"),
        col("customField.attributeName").alias("attributeName"),
        col("customField.value").alias("attributeValue")
      )
      .filter(col("attributeName").isNotNull && col("attributeValue").isNotNull)
    ...
    def createOrgCSV(orgId: String): Unit = {
      ...
      val orgData = explodedDF.filter(col("organisationId") === orgId)
      ...
      val pivotedData = orgData
        .groupBy("userID")
        .pivot("attributeName", attributeNames)
        .agg(first("attributeValue"))

      pivotedData.show(false)

      // Add organisationId back to the final dataset - Alternative approach
      val allColumns = Seq("organisationId", "userID") ++ attributeNames
      val finalDF = pivotedData
        .withColumn("organisationId", lit(orgId))
        .selectExpr(allColumns: _*)

      // Show sample data
      println(s"Sample data for org $orgId:")
      finalDF.show(5, truncate = false)

      // Write to CSV
      val outputPath = s"/home/analytics/shishir/output/org_${orgId}_custom_fields"
      finalDF
      ...
      println(s"CSV saved to: $outputPath")
    }
