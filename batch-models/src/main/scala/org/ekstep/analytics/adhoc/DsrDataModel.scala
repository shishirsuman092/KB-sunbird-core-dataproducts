...
object DsrDataModel extends AbsDashboardModel {
  ...
  implicit val className: String = "org.ekstep.analytics.dashboard.DsrDataModel"
  ...
  def processData(timestamp: Long)(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext, conf: DashboardConfig): Unit = {
    ...
    validate({
      userCourseProgramCompletionDF.count()
    }, {
      allCourseProgramCompletionWithDetailsDF.count()
    }, "userCourseProgramCompletionDF.count() should equal final course progress DF count")
  ...
  def processCertifications(courseProgramCompletionWithDetailsDF: DataFrame)(implicit spark: SparkSession, sc: SparkContext, fc: FrameworkContext, conf: DashboardConfig): Unit = {
    ...
    val totalCertificationsTillYesterday = if (totalCertificationsTillYesterdayStr == "") {
      0L
    } else {
      totalCertificationsTillYesterdayStr.toLong
    }
    val totalCertificationsTillDayBeforeYesterdayStr = Redis.get("lhp_certificationsTillYesterday")
    val totalCertificationsTillDayBeforeYesterday = if (totalCertificationsTillDayBeforeYesterdayStr == "") {
      0L
    } else {
      totalCertificationsTillDayBeforeYesterdayStr.toLong
    }
