...
object StatusReportModel extends AbsDashboardModel {
  ...
  try{
    ...
    def generateSurveyStatusReport(solutionId: String, solutionName: String) = {
      ...
      val batchSize = conf.SurveyStatusReportBatchSize.toInt
      getSolutionIdData(columnsToBeQueried, dataSource, solutionId, solutionName, batchSize)
      println(s"-------------- Successfully Generated Survey Status CSV In Batches And Combined Into A Single File For A SolutionId: ${solutionId} --------------")
    }
    ...
    def getSolutionIdData(columns: String, dataSource: String, solutionId: String, solutionName: String, batchSize: Int)(implicit spark: SparkSession, conf: DashboardConfig) {
      ...
      surveySubmissionIds.grouped(batchSize).toList.foreach { batchSurveySubmissionIds =>
        ...
        if (columnsMatch == true) {
          ...
          println(s"Batch : $batchCount, Successfully generated survey Status csv report for solutionId: $solutionId")
        } else {
    ...
    case e: Exception =>
      println(s"Error occurred during StatusReportModel processing: ${e.getMessage}", e)
      System.exit(1)
  }
  }
